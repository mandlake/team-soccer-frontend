...
function Home() {
  ...
  const question = [
    ...
    [
      '5-1',
      '수원팀에서 성이 고씨이고 키가 170 이상인 선수를 출력하시오. (ID 모를 경우)',
    ],
    [
      '6',
      '다음 조건을 만족하는 선수명단을 출력하시오. 소속팀(삼성블루윙즈 OR 드래곤즈)], 포지션(미드필더)], 키(170<=ALL<=180)',
    ],
    ...
    [
      '12',
      '수원팀(K02) 과 대전팀(K10) 선수들 중 키가 180 이상 183 이하인 선수들',
    ],
    [
      '13',
      '모든 선수들 중 포지션을 배정 받지 못한 선수들의 팀명과 선수이름 출력 둘다 오름차순',
    ],
    [
      '14',
      '팀과 스타디움, 스케줄을 조인하여 2012년 3월 17일에 열린 각 경기의 팀이름, 스타디움, 어웨이팀 이름 출력',
    ],
    [
      '15',
      '2012년 3월 17일 경기에 포항 스틸러스 소속 골키퍼(GK) 선수, 포지션,팀명 (연고지포함), 스타디움, 경기날짜를 구하시오',
    ],
    [
      '16',
      'STADIUM 에 등록된 운동장 중에서 홈팀이 없는 경기장까지 전부 나오도록',
    ],
    [
      '17',
      '페이지네이션 로직을 위한 플레이어 테이블에서 최상단 5개 로우를 출력',
    ],
    [
      '18',
        ...
        <tbody>
          {question.map((row: any) => (
            <tr>
              ...
              <td>{row[1]}</td>
              <td>
                <button value={row[0]} onClick={clickButtonHandler}>
                  답
                </button>
              </td>
            </tr>
